-- ============================================================================
-- 034. Cuenta corriente: cargo manual (deuda sin venta) + saldar deuda
-- vieja como parte de una venta nueva.
--
-- 1. `registrar_cargo_manual_cta_cte`: carga una deuda a mano (sin pasar por
--    el POS) para un cliente — ej. "Se seña turno $5000" antes de tener toda
--    la ficha del cliente completa. Mismo criterio que `registrar_pago_cta_cte`
--    pero tipo='venta' y sin fila en `ventas` (no hay ticket real detrás).
--
-- 2. `registrar_venta` gana `p_saldar_deuda`: si el cliente elegido en el POS
--    tiene saldo pendiente, ese monto se suma al total a cobrar EN LA MISMA
--    venta y se registra como un movimiento 'pago' que cancela la deuda vieja
--    — una sola transacción de caja en vez de dos pasos separados.
-- ============================================================================

create or replace function public.registrar_cargo_manual_cta_cte(
  p_tenant_id     text,
  p_cliente_id    uuid,
  p_monto         numeric,
  p_observaciones text default null
) returns public.cuenta_corriente_movimientos
language plpgsql
security definer
...
declare
  v_usuario   text;
  v_resultado public.cuenta_corriente_movimientos;
begin
  ...
  if not exists (select 1 from public.clientes where id = p_cliente_id and tenant_id = p_tenant_id) then
  ...
  insert into public.cuenta_corriente_movimientos
    (tenant_id, cliente_id, tipo, monto, observaciones, usuario_nombre)
  values
    (p_tenant_id, p_cliente_id, 'venta', p_monto,
     coalesce(nullif(trim(coalesce(p_observaciones, '')), ''), 'Cobro pendiente cargado a mano'), v_usuario)
...
create or replace function public.registrar_venta(
  ...
  p_saldar_deuda   numeric default 0
) returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if p_saldar_deuda is null or p_saldar_deuda = 'NaN'::numeric or p_saldar_deuda < 0 then
    raise exception 'El monto a saldar no es válido';
  end if;

  if p_saldar_deuda > 0 and p_cliente_id is null then
    raise exception 'Para saldar una deuda hace falta elegir el cliente';
  end if;
  ...
  v_total := greatest(v_suma - p_descuento, 0) + p_recargo + p_saldar_deuda;
  ...
  if p_medio_pago = 'cuenta_corriente' then
    ...
    values
      (p_tenant_id, p_cliente_id, 'venta', v_total - p_saldar_deuda, v_venta_id, v_usuario);
  end if;

  -- Saldar deuda vieja: se cobró junto con esta venta, se registra como pago.
  if p_saldar_deuda > 0 then
    insert into public.cuenta_corriente_movimientos
      (tenant_id, cliente_id, tipo, monto, venta_id, observaciones, usuario_nombre)
    values
      (p_tenant_id, p_cliente_id, 'pago', p_saldar_deuda, v_venta_id,
       'Saldado junto con la venta #' || v_numero, v_usuario);
  end if;
