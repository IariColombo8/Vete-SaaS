...
import { Package, History, TrendingDown, FileText, DollarSign, Boxes } from "lucide-react"
import {
...
import { TIPOS_AJUSTE, ETIQUETA_MOVIMIENTO } from "@/lib/productos/ajuste-stock"
...
interface Props {
  ...
  /** Solo aplica al alta (producto null): precarga el código escaneado. */
  codigoBarrasInicial?: string
  /** Pestaña con la que abre. Default "descripcion". */
  tabInicial?: Tab
  open: boolean
...
type Tab = "descripcion" | "precio" | "stock"
...
export function ProductoDialog({
  tenantId, producto, categoriasExistentes = [], codigoBarrasInicial, tabInicial, open, onOpenChange, onGuardar, onAjustarStock,
}: Props) {
  ...
  useEffect(() => {
    ...
    setTab(tabInicial ?? "descripcion")
    setAjusteTipo("entrada")
    setAjusteCantidad("")
    setAjusteNota("")

    if (!producto) {
      setCodigo(""); setCodigoBarras(codigoBarrasInicial ?? ""); setNombre(""); setCategoria(""); setImagenUrl("")
  ...
  }, [open, producto, tenantId, codigoBarrasInicial, tabInicial])
  ...
  // Sin stock (servicio) no hay nada que mostrar en el tab Stock.
  const tieneTabStock = controlaStock
  useEffect(() => {
    if (!tieneTabStock && tab === "stock") setTab("descripcion")
  }, [tieneTabStock, tab])
  ...
  const TABS: { value: Tab; label: string; icon: typeof FileText }[] = [
    { value: "descripcion", label: "Descripción", icon: FileText },
    { value: "precio", label: "Precio", icon: DollarSign },
    ...(tieneTabStock ? [{ value: "stock" as const, label: "Stock", icon: Boxes }] : []),
  ]
        ...
        <div className="space-y-4">
          ...
          {tab === "stock" && tieneTabStock && (
            <>
              ...
              {/* Mover stock: solo tiene sentido sobre un producto ya creado */}
